...
def upload_firmware(config_dir, iota_token, iota_url):
    ...
    with open(find_files("firmware.json", config_dir)[0], "r") as f:
        fw_json = json.load(f)
        headers["X-firmware-version"] = fw_json["version"]
        fw_file = os.path.join(config_dir, fw_json["file"])
    r = requests.put(upload_url, headers=headers, data=open(fw_file,
                                                            "rb"))
    if r.status_code == 201:
